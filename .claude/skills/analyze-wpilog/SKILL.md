...
Logs are written by DogLog through WPILib's `DataLogManager`, configured in
`Robot.configureLogging()`. `DataLogManager.makeLogDir()` prefers a mounted USB drive at `/u` when
one is writable, so files land in `/u/logs` with a flash drive plugged in and in
`<operating dir>/logs` without one. The chosen directory is logged to `/Robot/LogDir`.

`captureDs`, `logExtras`, and `captureConsole` are all on, so every log has the `/DS:` entries the
cheatsheet above relies on, plus CAN utilization, battery voltage, loop timing, and console output.
